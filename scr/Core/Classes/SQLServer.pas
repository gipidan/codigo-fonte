...
uses
   ...
   WinTypes, WinProcs, Tabs, ExtCtrls, DB, DBCtrls,StrUtils,
   Dialogs, StdCtrls, Messages, Graphics, shellapi, BDE, Sysutils,
   DBClient, SqlExpr, FMTBcd,  WideStrings, DBCommon,
   MemDS, Uni, DBAccess,
   Classe.Parametros,
   Global,
   Classe.Global;
...
Procedure prcViewFaturas(dbQueryView: TUniQuery;nCliente,nNFe:Integer);
...
//Procedure prcQueryCriar(tdbQuery: TUniQuery);
Procedure prcQueryCriar(var tdbQuery);
begin
   if (TUniQuery(tdbQuery) <> nil) then
      FreeAndNil(TUniQuery(tdbQuery));

   TUniQuery(tdbQuery) := TUniQuery.Create(Application);
   TUniQuery(tdbQuery).Connection := FrmPrincipal.DBConexao;
   TUniQuery(tdbQuery).Close;

end;

Procedure prcQueryDeletar(var tdbQuery);
begin
   if TUniQuery(tdbQuery)=Nil then   // se NÃO foi criado, nao presseguir.
      exit;
