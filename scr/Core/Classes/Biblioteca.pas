...
uses DBTables, Classes, Windows, Controls, Variants, ComCtrls,
   WinTypes, WinProcs, Tabs, ExtCtrls, DB, DBCtrls,
   Grids, Menus, Dialogs, StdCtrls, Messages, Graphics, WinSock,
   Printers, shellapi, MMSystem, BDE, Sysutils, DBGrids, BTOdeum,
   DBClient, SqlExpr, FMTBcd,
   frxClass, frxExportMail, AdvToolBar,
   MemDS, Uni, DBAccess, Character, Math, Forms,
...
Function Protegido(Tabela: TUniQuery): Boolean;

Function Centrar(FTextBuf: string; Cols: Integer): String;

Procedure Crosstab(aQuery: TUniQuery; aGrid: tStringGrid; ctColumn: Integer); overload;
Procedure CrossTab(dbDados:TUniQuery; stScriptSQL, stCampoLinhas, stTituloLinha, stCampoColunas , stCampoSomar, stWhere, sCampoOrderBy:String; stOperador:String='Sum'); overload;


Function ListarMenu(Menu: TMainMenu; Tag_Numero: Integer): TStrings;

Function ListarRibbonBar(Menu: TAdvToolBarPager; Tag_Numero: Integer): TStrings;
...
Procedure BeepCritica;
Var
   AvisoSonoro: TBTBeeper;
begin



   AvisoSonoro := TBTBeeper.Create(nil); // Criar na memoria
   //
   FrmPrincipal.AvisoSonoro.BeepFor(1000, 200); // fim

   AvisoSonoro.Free; // Destruir da memoria
...
begin
   AvisoSonoro := TBTBeeper.Create(nil); // Criar na memoria
   //
   FrmPrincipal.AvisoSonoro.BeepFor(600, 700); // fim
...
begin
   ...
   {
     Informar('Este módulo estará liberado após pagamento da linceça de uso...'+#13+#10+#13+#10+
     'atendimento@yyyy.com.br'+#13+#10+#13+#10+
     'www.yyyy.com.br');
...
Procedure Crosstab(aQuery: TUniQuery; aGrid: tStringGrid; ctColumn: Integer);
...
Procedure CrossTab(dbDados:TUniQuery; stScriptSQL, stCampoLinhas, stTituloLinha, stCampoColunas , stCampoSomar, stWhere, sCampoOrderBy:String; stOperador:String='Sum');
Var
...
begin
   ...
   // For x:= 0 to Menu.AdvPageCount -1 do
   For X := 0 to Menu.AdvPageCount - 1 do
   Begin

      if Menu.AdvPages[X].Caption <> '-' then
         Lista.Add(CampoTexto('==> ' + UpperCase(Menu.AdvPages[X].Caption) +
           ' <==', 40) + ' - ' +
           UpperCase(CampoTexto(Menu.AdvPages[X].Name, 30)));

      // For y:= 0 to Menu.AdvPages[x].AdvToolBarCount -1 do
      For y := 0 to Menu.AdvPages[X].AdvToolBarCount - 1 do

         // if Menu.AdvPages[x].AdvToolBars[y].Tag = Tag_Numero then
         if Menu.AdvPages[X].AdvToolBars[y].Tag = Tag_Numero then
         begin

            // if Menu.AdvPages[x].AdvToolBars[y].Caption<>'-' then
            if Menu.AdvPages[X].AdvToolBars[y].Caption <> '-' then
            Begin
               // Lista.Add(   CampoTexto( Menu.AdvPages[x].AdvToolBars[y].Caption,40) + ' - '+ UpperCase(CampoTexto(Menu.AdvPages[x].AdvToolBars[y].Name,30))  );
               Lista.Add(CampoTexto(Menu.AdvPages[X].AdvToolBars[y].Caption, 40)
                 + ' - ' + UpperCase(CampoTexto(Menu.AdvPages[X].AdvToolBars[y]
                 .Name, 30)));
            End;
